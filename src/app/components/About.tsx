...
export default function About() {
  ...
  const goToPrevious = () => {
    setCurrentFounderIndex((prev) => (prev - 1 + founders.length) % founders.length);
  };
...
<div className={`bg-[#edecec] border border-white/50 rounded-[40px] p-5 md:p-8 lg:p-10 relative shadow-[12px_12px_24px_#c9c9c9,-12px_-12px_24px_#ffffff] flex flex-col transition-all duration-1000 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`} style={{ transitionDelay: '500ms' }}>
    ...
    <div className="flex justify-center gap-2 mt-4">
      {founders.map((_, index) => (
        <button
          ...
          aria-label={`Go to ${founders[index].name}`}
        />
      ))}
    </div>
  </div>
